...
- 文件夹模式：按文件名生成伪 UID（`local-N`），无发票号时以 **文件名 + 金额** 作为去重键（避免同名无号发票被误并）。
...
打车/火车类发票，金额销售方来自**发票 PDF**，起点终点来自同邮件/同文件夹的**行程单 PDF**。step4 的多附件合并逻辑：一封邮件/文件夹内的多个 PDF，行程单的 `fromStation/toStation` 覆盖发票的空值；`tripUncertain` 用显式布尔判断（避免 `false || null` 被吞）。
