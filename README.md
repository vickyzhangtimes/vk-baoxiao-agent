...
当前版本：`v2.0.0 Release Candidate` · Node.js `>=18` · Windows / macOS / Linux · `103/103` tests
...
```text
用报销发票 Agent 处理 E:\本月报销，沿用我的公司模板和分类规则，
...
```powershell
git clone https://github.com/vickyzhangtimes/reimbursement-invoice-agent.git
cd reimbursement-invoice-agent
npm install
npm run init
```
...
```text
使用 reimbursement-invoice-agent，先展示权限计划，
再整理我指定文件夹中的发票和行程单。
