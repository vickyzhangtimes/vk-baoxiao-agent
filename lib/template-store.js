...
const { assertSafeChild, safeSegment } = require('./path-guard');
...
function templateBaseDir(user, name) {
  ...
  const target = path.join(root, safeSegment(user, 'template user'), safeSegment(name, 'template name'));
  return assertSafeChild(root, target, '模板目录');
}
...
function listTemplates(user) {
  const root = templatesRoot();
  const u = assertSafeChild(root, path.join(root, safeSegment(user, 'template user')), '模板用户目录');
