...
function applyOverrides(records, dateTag) {
  ...
  for (const o of ov.items) {
    ...
    for (const field of ['transportType', 'tripDate', 'fromStation', 'toStation', 'note']) {
      if (o[field]) r[field] = String(o[field]).trim();
    }
    if (Array.isArray(o.legs) && o.legs.length) {
      r.legs = o.legs;
      r.fromStation = o.legs[0].from || r.fromStation;
      r.toStation = o.legs[0].to || r.toStation;
      r.tripUncertain = false;
    }
    const confirmsRecord = amt != null || o.invoiceNo || o.invoiceDate || o.buyer || o.archivePath || o.fromStation || o.toStation || (o.legs && o.legs.length);
    if (confirmsRecord) { r.needsManualReview = false; r.manualReason = null; }
    r.overridden = true;
    applied++;
  }
